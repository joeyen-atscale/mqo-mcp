...
**Blank-member answer fidelity** (PRD-mqo-blank-member-answer-fidelity). When a `query_multidimensional` result contains rows whose dimension member is blank/NULL (the "unknown" member — records with no value for that attribute), the response now surfaces an explicit `blank_member_rows` count plus an advisory note in `notes[]`, so the consuming LLM keeps the blank row in its answer instead of silently dropping it. This closes the `product-count-per-category` failure where the tool returns 11 rows incl. a null-category row but the model reports 10. Controlled by `--unknown-member-mode {annotate|caption|off}` (env `MQO_UNKNOWN_MEMBER_MODE`, default `annotate`): `annotate` adds the count + note and leaves rows unchanged (blank cells stay NULL, so gold scoring is unaffected); `caption` additionally rewrites blank dimension cells in the INLINE rows with `--unknown-member-caption` (default `(blank)`), leaving handle/summary untouched; `off` is byte-identical to the pre-feature response. No-op when a result has zero blank members (FR4). The count in the cursor/handle path is computed over the FULL materialized result, not just the first page (FR7). New `UnknownMemberMode` enum (re-exported from the lib); blank-member counting/captioning helpers in `handle_ops.rs`; 7 new unit tests (4 response-level in `mcp.rs`, 3 helper-level in `handle_ops.rs`).
...
**Inline ontology-check enforcement stage in `query_multidimensional`** (PRD-swa-ontology-check-inline-enforcement). Promotes the advisory `validate_query_ontology` tool into the execution hot path: a new `ontology_check_inline()` stage now runs automatically alongside `param_validate()` for every `query_multidimensional` call, with no agent action required. Configurable via `--ontology-check-mode {off|warn|enforce}` (env `MQO_ONTOLOGY_CHECK_MODE`, default `warn`). In `warn` mode: findings are attached as advisory metadata, query proceeds. In `enforce` mode: findings with `severity: error` reject the query pre-execution via the existing `PipelineError::ParamRejected` envelope (same retry path the agent already handles). Per-rule disable via `--ontology-check-disable-rule <rule_id>` (repeatable; env `MQO_ONTOLOGY_CHECK_DISABLED_RULES` comma-separated). Fail-open: when no model graph is loaded (autolift disabled or graph unavailable), the stage skips transparently — no query ever blocked for lack of a graph. FR6 structured log line emitted on every call: `ontology_checked=true|skipped`, rules fired, disposition. Zero false rejections on the C10 passing baseline (NFR2 guardrail, verified by AC7 test). New `OntologyCheckMode` enum in `mcp.rs`; 7 new pipeline unit tests (AC1–AC5 + FR5 + edge). Total tests: +7.

## v0.45.0 — 2026-06-16

**New tool: `query_model_graph`** — read-only, budgeted canned-query surface over the lifted model RDF knowledge graph (PRD-swa-model-sparql-tool). Answers structural questions that `describe_model` cannot: hierarchy rollup levels (ordered coarse→fine with IRIs + labels), calc/measure lineage (`aso:dependsOn` edges), role-playing dimension references (`aso:playsRoleOf`), and cross-model conformance (`owl:sameAs` — populated by the future lattice-bridge OSL #7 tier). Implemented as direct triple-pattern traversal over an `oxrdf::Graph` (same `oxrdf` 0.3 crate already in the workspace via `aso-tbox`/`aso-lift`); no SPARQL engine required for the v1 canned-query set. Key properties: raw SPARQL off by default (operator opt-in, AC5); no-graph → `model_graph_not_available` (AC6); over-budget → typed `budget_exceeded`, never hangs (AC4); unknown query name / bad params → actionable error listing valid queries and param schema (AC8). Results contain only model-metadata IRIs/literals, never warehouse row data (AC7). **Live auto-lift tier not yet deployed**: live and fixture-mode servers return `model_graph_not_available` until the OSL #2 auto-lift pipeline populates the graph at startup. Tests load a fixture lifted `.ttl` directly to exercise all ACs (AC1–AC8; AC3 stubbed as empty result — correct behavior until `lattice-bridge` is built). 12 model_graph unit tests, all green. Total tool count: 25 (was 24).

## v0.44.0 — 2026-06-15

**Fix: projection cardinality guard now applies a filter on a non-projected level of the *same* hierarchy** (gap found by API-free validation). A `Range`/`Member` filter that targets a different level of the projected hierarchy (e.g. `Product Current Price > 70` filtering an `Item Product Name` projection — both `product_dimension`) was ignored by the estimate: `single_filter_estimate`'s `Range` arm only matched a filter on the *projected* level, and the cross-hierarchy (FR-5) path skipped it because the hierarchy *is* projected. So `products-price-above-70` estimated the full 206,021-row domain and was wrongly rejected with `projection_too_large` despite a true result of 6,854. Fix: when a `Range` filter targets a non-projected level of the projected hierarchy, apply the same conservative 1/10 selectivity (FR-5) to the group base; the runtime materialization budget (`row_cap_tripped`) remains the hard bound. New test `range_on_non_projected_level_of_same_hierarchy_reduces_estimate`. **Also redeployed the stale pipeline helper binaries** (`mqo-bind`/`route`/`dax`/`mdx` were Jun-14, predating the v0.43 cross-hierarchy decline) — the v0.43 `member_unbound_cross_hierarchy` check now actually fires on the live `corpcorp` path (typed decline instead of an engine 500). Known remaining: `products-price-above-70` now clears the guard but is blocked one layer deeper — `Product Current Price` is typed String, so a numeric range bound is rejected by the filter-level type validator (range-filter-native-typing / catalog-typing issue, tracked separately).

## v0.38.0 — 2026-06-15

**Handle full materialization — the persisted handle holds the whole result, not the first 1000 rows** (PR #15, `fix/handle-full-materialization`). The auth-bridge clamped every result to `HARD_ROW_CAP = 1000` (`engine.rs`) — fetching `cap+1` and truncating to 1000 *before* the handle was persisted — so a large result (e.g. a 9,859-row "list each customer" answer) became a 1000-row handle and every `dataset_*`/`dataset_export` over it was silently lossy (eval `customers-ese-store-2001` resolved 1000 != 9859 gold). Fix: a configurable `--max-result-rows` materialization budget (default 50,000, clamped to the 200,000 PGWire ceiling) replaces the hard-coded cap at both the fetch limit and the truncation, so the persisted handle holds the full result up to the budget. The inline `query_multidimensional` sample stays bounded by `inline_threshold` (decoupled — raising the budget does not enlarge LLM context). Over-budget results set `row_cap_tripped` and surface a typed `result_too_large` signal (budget value + `truncated: true`) on both the inline and cursor paths rather than silently clamping. A caller-supplied `limit` is now an intentional bound that does not trip the flag. `dataset_export`'s JSON default raised to 50,000 to match (no second silent clamp). DuckDB handle store has no independent row cap (only a byte budget), so no store change. Also clears two pre-existing workspace-test breaks (`dh-mcp-server` non-exhaustive `PipelineError` match; `dh-spec` stale capability count). Prerequisite for the handle-first contract + mcp-eval handle-scoring to work for results >1000.

## v0.37.0 — 2026-06-15

**Fix: near-twin dimension levels now emit canonical output labels (G2)** (PR #14, `fix/near-twin-canonical-binding`). The model exposes near-twin hierarchies where the same logical attribute appears under multiple hierarchies with prefixed captions — e.g. `promotion_product_item_product_dimension.[Promotion Product Item Item Product Name]` and `store_item_product_dimension.[Store Item Product Category]` alongside the base `product_dimension.[Item Product Name]` / `[Product Category]`. When a query projected a near-twin level, the result column was named after the prefixed caption verbatim, failing an exact-name comparison against the canonical attribute (eval cases `corpcorp-brand-products`, `product-count-per-category`). Fix: `canonical_level_label` collapses a near-twin caption to the longest proper token-suffix that is itself a level caption elsewhere in the model (pure label logic — needs no catalog `domain`/`value_type` metadata); `pipeline::attach_canonical_dimension_labels` writes the canonical label onto each bound dimension after `bind_step`, and `clean_result_rows` emits it. New tests: `canonical_level_label_collapses_near_twins`, `clean_result_rows_emits_canonical_near_twin_label`, and four `pipeline::canonical_label_tests`. **Known limitation:** the cross-hierarchy *filter*-binding half of G2 remains blocked — only ~25/231 level columns carry a captured `domain` and nothing marks twins as canonical copies, so there is no sound deterministic signal; the PR #11 dimension-completeness guard remains the honest-failure floor. Also fixes a pre-existing test-build break: four `Mqo` literals in `mqo-mdx-compiler` tests lacked the `projection` field added in v0.35.0.

## v0.36.0 — 2026-06-14

**Fix: banded-tier cell values no longer stripped of leading spaces** (`mqo-auth-bridge` v0.4.1). The XMLA tabular-rowset parser called `reader.config_mut().trim_text(true)` and additionally applied `.trim()` to each cell value. This stripped the meaningful leading spaces from banded-level members such as `"  0- 50"` → `"0- 50"` and `" 50-100"` → `"50-100"`, producing value mismatches against the gold SQL for any query projecting `Net Profit Tier` (eval case `net-profit-tier-by-store-gender-2002`). Fix: removed `trim_text(true)` from both the tabular-rowset and MDDataSet parsers; cell values are now passed to `coerce_cell` verbatim. The `coerce_cell` function is updated to treat whitespace-only text as `Null` (indentation nodes) while preserving leading spaces in non-empty string values. MDDataSet caption text (column/hierarchy names) is still trimmed explicitly since column names should not have leading/trailing whitespace. New tests: `tabular_banded_tier_leading_spaces_preserved`, `coerce_cell_whitespace_only_is_null`, `coerce_cell_leading_space_preserved_in_string`, `coerce_cell_numeric_with_surrounding_spaces_parses_as_number`.

## v0.35.0 — 2026-06-14

**`hierarchy_levels` now carries `value_type` for numeric levels** (fix/store-employee-attribute-projection). The `describe_model` `hierarchy_levels` block previously omitted the `value_type` field from level entries, stripping the catalog's `value_type:"integer"` annotation. Without this signal the LLM could not distinguish a projectable integer-valued attribute (e.g. "Store Number of Employees", a catalog `kind=level`) from an aggregatable measure, and incorrectly generated `SUM("Store Number of Employees")` instead of projecting the attribute. Fix: when a level column in the catalog carries a `value_type` field (e.g. `"integer"`, `"string"`, `"date"`, `"decimal"`), it is now passed through to the `hierarchy_levels` entry unchanged. Levels without a `value_type` in the catalog emit no `value_type` key (absent is unambiguous, null is not). Additive change — no existing fields altered. Three regression tests guard the fix: integer passthrough, string passthrough, and no-key-when-absent. The `store-employee-counts` eval case (`"For each store name, how many employees work there?"` → project `Store Number of Employees`, not `SUM`) should now ground correctly.

## v0.34.0 — 2026-06-15

**Dimension-completeness guard** (PRD-mqo-near-twin-dimension-drop, G1). When the DAX engine silently drops a requested dimension column from the result — observed live (v0.33.0) with `Product Category`, a near-twin level shared across `product_dimension` and `promotion_product_item_product_dimension` — the server previously returned measure-only rows that the LLM would mislabel. This release adds a `DimensionNotMaterialized` pipeline error and a post-execution dimension-completeness guard (mirroring the existing measure-completeness guard at `pipeline.rs:488`). After execution, if the first result row has fewer non-measure columns than the number of requested dimensions, the server returns a typed `dimension_not_materialized` error carrying `requested_dimensions`, `dimension_columns_returned`, and `compiled_query` — never silently returns measure-only rows. Guard is gated `is_live && backend == "dax"` exactly like the measure guard (fixture mode unaffected). New helper `dax_dim_column_count` counts non-`_x005b_`-prefixed columns in a result row. Four new unit tests: `counts_dim_columns_not_measures`, `near_twin_drop_yields_zero_dim_cols`, `dim_guard_all_dims_present_passes`, `dim_guard_missing_dim_returns_dimension_not_materialized`. **G2 (root cause) status:** investigated statically; the drop mechanism is entangled with live catalog-ingest state (fresh MDSCHEMA ingest vs disk cache) and cannot be reproduced or fixed deterministically in a worktree without the live bridge. Static analysis implicates `catalog_ingest.rs` OQ-5 dual-registration of near-twin levels and the per-level `tables` map in `DaxCatalogContext` — a near-twin may resolve to a different physical table name than the engine returns. Root cause left as OQ-1/OQ-2 for the engineering dig with live catalog snapshots.

## v0.33.0 — 2026-06-14

**`dataset_*` ops accept canonical clean column names** (PRD-mqo-dataset-op-clean-args). Every `dataset_*` op now documents and enforces the canonical column-name contract: column args (`group_by`, predicate `col`, sort key, `measure`, `x_col`, `y_cols`, `date_col`, `measure_cols`, etc.) must use the exact names returned by `query_multidimensional` or `dataset_describe`. Tool schemas updated with explicit guidance (FR-2). A typed `unknown_column` error lists the handle's actual canonical columns so the LLM can self-correct in one turn (FR-5). Back-compat: a legacy raw XMLA key passed as a column arg is still resolved via `clean_label`/case-insensitive match, with a deprecation warning surfaced in the response `warnings` field (FR-4). Round-trip invariant test: every column name from a simulated `query_multidimensional` response is accepted unchanged by `dataset_sort`, `dataset_filter`, `dataset_aggregate`, `dataset_top_n`, `dataset_slice`, `dataset_period_over_period`, `dataset_chart`, and `dataset_export` (AC-3/FR-3 guardrail). Op semantics unchanged (FR-6). (PRD-mqo-dataset-op-clean-args)

## v0.32.0 — 2026-06-14

**Handle stores canonical clean column labels** (PRD-mqo-handle-canonical-labels). The `query_multidimensional` result handle now persists columns under the same canonical clean names the response uses (`Year`, `Revenue`) instead of raw DAX-mangled backend keys (`atscale_catalogs_x005b_Sold_x0020_Calendar_x0020_Year_x005d_`). The handle schema, `dataset_*` op column references, and `dataset_export` output now all share one canonical identity — the same `clean_result_rows` function (FR-3 single shared impl) that was already cleaning the response, now applied before persisting. Collision disambiguation is identical between response and handle (FR-5). Idempotent: already-clean names pass through unchanged (FR-7). Back-compat: the `clean_result_rows` resolver remains available for callers passing legacy raw keys (FR-6). This is the foundation for `dataset_*` ops and chart tools to reference one consistent column name without translation. (PRD-mqo-handle-canonical-labels)
