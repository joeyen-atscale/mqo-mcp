...
# Count AC results. This project uses a single tests/acceptance.rs with
# functions named ac1_*, ac2_*, etc. (not split-by-file convention).
# Support both: count acceptance_*.rs files (split convention) AND
# ac[0-9]+_* test functions inside acceptance.rs (monolithic convention).
...
# Use whichever count is non-zero; prefer split-file if both are non-zero.
