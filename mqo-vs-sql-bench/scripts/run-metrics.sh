...
# Count AC results. Supports two conventions:
#  - Split-file: tests/acceptance_*.rs (one file per AC)
#  - Monolithic: tests/acceptance.rs with fn ac[N]_* test functions
# Both patterns are detected; split-file takes priority if both are non-zero.
# (auto-evolved from mqo-mcp-server run 2026-06-08)
AC_TOTAL_FILES=$(find tests -maxdepth 1 -name 'acceptance_*.rs' -type f 2>/dev/null | wc -l)
AC_TOTAL_FNS=$(grep -cE '^fn ac[0-9]+_' tests/acceptance.rs 2>/dev/null || echo 0)
# Also detect ac[N]_* in tests/integration.rs (mqo-vs-sql-bench convention)
AC_TOTAL_INTEGRATION=$(grep -cE '^fn ac[0-9]+_' tests/integration.rs 2>/dev/null || echo 0)
if [ "$AC_TOTAL_FILES" -gt 0 ]; then
  AC_TOTAL="$AC_TOTAL_FILES"
elif [ "$AC_TOTAL_FNS" -gt 0 ]; then
  AC_TOTAL="$AC_TOTAL_FNS"
else
  AC_TOTAL="$AC_TOTAL_INTEGRATION"
fi
AC_PASSING=0
AC_RESULTS='[]'
if [ "$AC_TOTAL" -gt 0 ]; then
  # Split-file convention: test acceptance_ac1 ... ok
  PASSING_SPLIT=$(grep -cE '^test acceptance_[a-z0-9_]+ \.\.\. ok' target/autobuilder/test-output.txt || true)
  # Monolithic convention: test ac1_* ... ok
  PASSING_FNS=$(grep -cE '^test ac[0-9]+_[a-z0-9_]+ \.\.\. ok' target/autobuilder/test-output.txt || true)
  AC_PASSING=$(( PASSING_SPLIT + PASSING_FNS ))
fi
