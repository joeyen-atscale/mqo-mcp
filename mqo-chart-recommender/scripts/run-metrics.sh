...
AC_TOTAL=$(find tests -maxdepth 1 -name 'ac*.rs' -type f 2>/dev/null | wc -l | tr -d ' ')
AC_PASSING=0
if [ "$AC_TOTAL" -gt 0 ]; then
  AC_PASSING=$(grep -cE '^test ac[0-9]+_.+ \.\.\. ok' target/autobuilder/test-output.txt 2>/dev/null || true)
fi
...
if [ -f agent/intent-card.json ]; then
  INTENT_METRIC_NAME=$(jq -r '.unfakeable_metric.name // "acceptance_tests_passing_count"' agent/intent-card.json 2>/dev/null)
fi
